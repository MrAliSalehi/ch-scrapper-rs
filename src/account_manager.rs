...
pub async fn check_status(client_handler: &Client, signed_in: Result<User, SignInError>) {
    match signed_in {
        Err(SignInError::PasswordRequired(password_token)) => {
            ...
            client_handler.check_password(password_token, password.trim()).await.unwrap();
        }
        Ok(user) => {
...
pub async fn sign_in_async(config: &AppConfig, client_handler: &Client) -> Result<User, SignInError> {
